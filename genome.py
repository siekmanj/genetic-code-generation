...
class Genome:
    ...
    def crossbreed(self, other_genome, mutation_rate): #takes two existing genomes, and inserts a word from one of two parents into every slot.
        ...
        for word in range(0, len(new_genome.genome)-1): #iterate through every word so that each one has an equal chance of being mutated
            ...
            if(randint(0, 100) < mutation_rate * 100): #random chance of a character in a word mutating into something else
                
                mutation = list(new_genome.genome[word])
                mutation_type = randint(1, 4)
                
                if mutation_type == 1: #25% chance that the mutation will be a character replacement
                ...
                elif mutation_type == 2 and len(new_genome.genome[word]) > MIN_WORD_LEN: #25% chance that the mutation will be a character deletion
                    
                    del mutation[randint(0, len(new_genome.genome[word])-1)] #delete a random character from the word
                    
                elif mutation_type == 3 and len(new_genome.genome[word]) < MAX_WORD_LEN: # 25% chance that the mutation will lengthen the string by one character
                    
                        mutation.append(choice(characters)) #append a random character onto the end of the word
                        
                elif mutation_type == 4 and len(new_genome.genome[word]) > MIN_WORD_LEN: # 25% chance that the mutation will shorten the string by one character.
                    
                        mutation.pop() #remove the last character from the word
