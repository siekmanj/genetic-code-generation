...
def fitnessWrapper(array, index, genomes):
    array[index] = genomes[index].fitness()

def fillList(array, length):
...
def printGenome(genome):
    ...
    for i in genome.genome:
        ...
        if counter % 15 == 0:
            print("")
def main():
    genomes = initializeGenomes(genome.INITIAL_GENOMES)
    numGenomes = genome.STABLE_GENOMES
    ...
    while(True):
        ...
        for i in range(0, randint(0, len(genomes)-1)):
            genomes.append(genomes[randint(0, len(genomes)-1)].crossbreed(genomes[randint(0, len(genomes)-1)], mutation_rate))
        ...
        if(similarity_factor > .2 and mutation_rate < .85):
            mutation_rate += 0.005
        if(similarity_factor < .2 and mutation_rate > .3):
            mutation_rate -= 0.005
        

        print("Genetic similarity between #1 and top " + str(len(cream_of_the_crop)) + ": " + str(math.floor(100 * similarity_factor)) + "%. ", end="")
        print("Highest score was 0." + str(math.floor(1000*top_fitness)) + " with a keyword count of " + str(top_wordcount) + "/250 (" + str(100*(top_wordcount)/250) + "%) and " + str(cream_of_the_crop[0].getMutations()) + " mutations.")
        time.sleep(2)
